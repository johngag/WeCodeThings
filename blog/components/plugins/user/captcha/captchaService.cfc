...
<cfcomponent
	...
	<cffunction name="loadXML" access="private" returntype="void" output="false"
		...
		<cfloop from="1" to="#ArrayLen(fontNodes)#" index="i">
			<cfif fontNodes[i].XmlAttributes['use']>
				...
				<cfset fontLocation = findInArray(allFontNames, tempName) />
				
				<!--- Use the font if it is available on the system --->
				<cfif fontLocation>
					<cfset ArrayAppend(definedFonts, allFonts[fontLocation]) />
				</cfif>
			</cfif>
		</cfloop>
	...
	<cffunction name="findInArray" access="private" returntype="numeric" output="false"
	...
	<cffunction name="setConfigFile" access="private" returntype="void" output="false">
		...
		<!--- changed by Laura Arguello to remove expandpath --->
