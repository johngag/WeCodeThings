...
<cfcontent reset="true" /><cftry><cfinclude template="#blog.getSetting('skins').path##blog.getSkin()#/#request.pageTemplate#"><cfcatch type="missinginclude"><cfinclude template="#blog.getSetting('skins').path##blog.getSkin()#/page.cfm"></cfcatch></cftry>